...
def VisualGraphs(RstActual,RstBest,ZTuckerRepr,ConvCurve,Original,ColorBands,H_cr,H_re,rho,iters):
    ...
    cr='Y'+H_cr.split('/')[2].split('n')[1].replace('.mat','')
    re='H'+H_re.split('/')[2].split('n')[1].replace('.mat','')
    title=cr+'_'+re+'_'+str(rho)+'_'+str(iters)
    ...
    name=title+'_%.3f'%np.max(ConvCurve)+'.png'
...
def UNetL(pretrained_weights = None,input_size = (256,256,1), L=10, H=0, fact = 0.5):
    ...
    conv8 = Conv2D(L, 1,name='convF_red1')(conv7)
...
def UNetL2(pretrained_weights=None, input_size=(256, 256, 1), L=10, H=0, fact=0.5):
    L_2 = 2 * L
    L_3 = 3 * L
    L_4 = 4 * L
    ...
    f1 = Lambda(lambda x: Hxfunction(x, largo=input_size[0], ancho=input_size[1], profun=L, H=H),name='f1')(conv8)

    conv9 = Conv2D(L, 3, activation='relu', padding='same', kernel_initializer='he_normal',name='conv1_red2')(conv8)

    f2 = Lambda(lambda x: Hxfunction(x, largo=input_size[0], ancho=input_size[1], profun=L, H=H),name='f2')(conv9)

    model = Model(inputs, [f1,f2])
...
class myCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if np.mod(epoch,Freq)==0:
            ...
            func = K.function([self.model.layers[0].input],[self.model.get_layer('convF_red1').output])
